    ...
    try:
        ...
        if not model_exists:
            openai.Deployment.create(
                model=oai_model, scale_settings={"scale_type": "standard"}
            )
            time.sleep(30)
        ...
        while counter < 2:
            ...
            for deployment in result.data:
                logging.info(
                    f"OpenAI Deployment Exception --- Found deployment {deployment}"
                )
                if (deployment["status"] == "succeeded") and (
                    deployment["model"] == oai_model
                ):
                    deployed = True
...
def get_openai_embedding(query, embedding_model=CHOSEN_EMB_MODEL):
    return openai.Embedding.create(input=query, engine=embedding_deployment_id)["data"][
        0
    ]["embedding"]
...
def openai_summarize(
    text, completion_model, max_output_tokens=MAX_OUTPUT_TOKENS, lang="en"
):
...
def contact_openai(
    ...
    try:
        ...
        if (gen == 4) or (gen == 3.5):
            ...
            if verbose:
                print(
                    f"Using GPT-4 - Chat Completion - with stream {stream} - OpenAI response time: {a-b}"
                )
            if stream:
                return resp
            else:
            ...
            if verbose:
                print(
                    f"Using GPT-3 - Chat Completion - with stream {stream} - OpenAI response time: {a-b}"
                )
            if stream:
                return resp
            else:
