...
from utils import http_helpers, openai_helpers
...
class CV:
    ...
    def process_json(self, img_url, response):
        ...
        res["captions"] = [
            caption["text"] for caption in response["denseCaptionsResult"]["values"]
        ]

        res[
            "text"
        ] = f"[{img_url}] This is an image. Main Caption: {res['main_caption']}\nOCR: {res['ocr']}\nDense Captions: {', '.join(res['captions'])}\nTags: {', '.join(res['tags'])}"
    ...
    def analyze_image(self, img_url=None, filename=None):
        if filename is not None:
            ...
            response = self.http_req.post(
                op="analyze", headers=self.http_req.json_headers, body={"url": img_url}
            )
    ...
    def get_img_embedding(self, img_url=None, filename=None):
        if filename is not None:
            ...
            response = self.http_req.post(
                op="img_embedding",
                headers=self.http_req.json_headers,
                body={"url": img_url},
            )
    ...
    def get_text_embedding(self, text):
        response = self.http_req.post(
            op="text_embedding", headers=self.http_req.json_headers, body={"text": text}
        )
