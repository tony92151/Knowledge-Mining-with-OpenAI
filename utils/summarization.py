...
from utils import fr_helpers, helpers, openai_helpers
...
def chunk_doc(
    ...
    if mode == "refine":
        max_tokens = (
            max_tokens
            - len(enc.encode(refine_prompt_template))
            - len(enc.encode(refine_template))
            - 2 * MAX_OUTPUT_TOKENS
            - chunk_overlap
        )
        ...
        max_tokens = (
            max_tokens
            - len(enc.encode(mapreduce_prompt_template))
            - MAX_OUTPUT_TOKENS
            - chunk_overlap
        )
    else:
    ...
    text_splitter = TokenTextSplitter(
        encoding_name=enc_name, chunk_size=max_tokens, chunk_overlap=chunk_overlap
    )
...
def get_mapreduced_summarization(
    docs,
    model=CHOSEN_COMP_MODEL,
    max_output_tokens=MAX_OUTPUT_TOKENS,
    stream=False,
    ...
    PROMPT = PromptTemplate(
        template=mapreduce_prompt_template, input_variables=["text"]
    )
...
def read_document(path, verbose=False):
    ...
    if ext == ".xlsx":
        ...
        for sheet in sheets:
            print("sheet", sheet)
            all_text += (
                pd.read_excel(path, sheet_name=sheets[0]).to_string(na_rep="")
                + "\n\n\n\n"
            )
    elif ext == ".csv":
        return None
        ...
        (
            ...
            dfs,
            t_contents,
        ) = fr_helpers.fr_analyze_local_doc_with_dfs(path, verbose=verbose)
...
def summarize_document(path, mode="refine", verbose=False):
    ...
    print(
        f"Done Processing {path} in {end-start} seconds\n##########################\n"
    )
    return summary
...
def summarize_folder(
    folder, mode="refine", save_to_csv=True, save_to_pkl=True, verbose=False
):
    ...
    if os.path.exists(csv_file):
        ...
        summ_df = pd.DataFrame(
            columns=["file", "intermediate_steps", "summary", "proc_time"]
        )
    ...
    for f in files:
        ...
        if save_to_csv:
            summ_df.to_csv(csv_file)
        if save_to_pkl:
            summ_df.to_pickle(pkl_file)
