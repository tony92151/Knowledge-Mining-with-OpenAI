...
class HTTPRequest:
    ...
    def initialize_for_cogsearch(
        self, api_key, search_service_name, index_name, api_version
    ):
        self.api_key = api_key
        ...
        self.url = (
            f"{search_service_name}/indexes/{index_name}?api-version={api_version}"
        )
...
class CVHttpRequest(HTTPRequest):
    def __init__(
        ...
        options = (
            ",".join(options).replace(" ", "") if isinstance(options, list) else options
        )
