...
from utils import cv_helpers, helpers, openai_helpers, redis_helpers, storage
from utils.cogsearch_helpers import cog_lookup, cog_search, cog_vecsearch
from utils.cogvecsearch_helpers import cogsearch_vecstore
from utils.env_vars import *
from utils.helpers import redis_lookup, redis_search
from utils.langchain_helpers import streaming_handler
from utils.langchain_helpers.mod_agent import (ModBingSearchAPIWrapper,
                                               ModConversationalChatAgent,
                                               ReAct, ZSReAct)
...
class KMOAI_Agent:
    def __init__(
        ...
        if self.enable_unified_search == None:
            self.enable_unified_search = False
        if self.enable_cognitive_search == None:
            self.enable_cognitive_search = False
        if self.enable_redis_search == None:
            self.enable_redis_search = False
        if self.evaluate_step == None:
            self.evaluate_step = False
        if self.check_adequacy == None:
            self.check_adequacy = False
        if self.check_intent == None:
            self.check_intent = False
        if self.use_calendar == None:
            self.use_calendar = False
        if self.use_calculator == None:
            self.use_calculator = False
        if self.use_bing == None:
            self.use_bing = False
        ...
        if self.verbose:
            print("use_calendar", self.use_calendar)
        if self.verbose:
            print("use_calculator", self.use_calculator)
        if self.verbose:
            print("use_bing", self.use_bing)
        ...
        if force_redis:
            if (
                (self.enable_unified_search == False)
                and (self.enable_cognitive_search == False)
                and (self.enable_redis_search == False)
                and (self.use_bing == False)
            ):
        ...
        if connection == None:
            ...
            callbacks = [
                streaming_handler.StreamingSocketIOCallbackHandler(
                    connection["socketio"], connection["connection_id"]
                )
            ]

        self.llm = helpers.get_llm(
            CHOSEN_COMP_MODEL,
            temperature=0,
        ...
        if self.use_calculator:
            agent_tools += [
                Tool(
                    name="Calculator (only math formulas, NO text search)",
        ...
        if self.use_calendar:
            agent_tools += [
                Tool(
                    name="Calendar",
                    func=self.get_date,
                    description="useful for when you need to get the current time and date to answer the question. USE ONLY IF THIS IS RELEVANT TO THE QUESTION.",
                )
            ]
        ...
        if self.enable_unified_search:
            agent_tools += [
                Tool(
                    name="Knowledge Base Search #1",
                    func=self.unified_search,
                    description="useful for when you need to start a search to answer questions from the knowledge base",
                )
            ]
        ...
        if self.enable_redis_search:
            agent_tools += [
                Tool(
                    name="Knowledge Base Search #3",
                    func=self.agent_redis_search,
                    description="useful for when you need to answer questions from the Redis system",
                ),
            ]
        ...
        if self.enable_cognitive_search:
            agent_tools += [
                Tool(
                    name="Knowledge Base Search #4",
                    func=self.agent_cog_search,
                    description="useful for when you need to answer questions from the Cognitive system",
                ),
            ]
            ...
            if USE_COG_VECSEARCH != 1:
                agent_tools += [
                    Tool(
                        name="Knowledge Base Search #2",
                        func=self.agent_cog_lookup,
                        description="useful for when you need to search for named entities from the the Cognitive system",
                    ),
                ]
        ...
        if self.use_bing or (USE_BING == "yes"):
            ...
            agent_tools.append(
                Tool(
                    name="Online Search",
                    func=self.agent_bing_search,
                    description="useful for when you need to answer questions about current events from the internet",
                ),
            )
        else:
        ...
        if PROCESS_IMAGES == 1:
            agent_tools += [
                Tool(
                    name="Analyse Image",
                    func=self.agent_analyze_image,
                    description="useful for when you need to analyze images and get back text description",
                ),
                Tool(
                    name="Get Similar Images with URL",
                    func=self.agent_get_similar_images,
                    description="useful for when you need to get similar images from the knowledge base",
                ),
            ]
        ...
        self.zs_chain = AgentExecutor.from_agent_and_tools(
            self.zs_agent,
            agent_tools,
            verbose=verbose,
            max_iterations=4,
        ...
        self.ccrd_agent = ModConversationalChatAgent.from_llm_and_tools(
            self.llm, agent_tools
        )
        self.ccrd_chain = AgentExecutor.from_agent_and_tools(
            self.ccrd_agent,
            agent_tools,
            verbose=verbose,
            max_iterations=4,
        ...
        zs_pr = self.zs_agent.create_prompt([]).format(
    ...
    def agent_redis_search(self, query):
        response = redis_helpers.redis_get(
            self.redis_conn, query, "redis_search_response", verbose=self.verbose
        )

        if response is None:
            response = "\n\n".join(redis_search(query, self.redis_filter_param))
            response = self.evaluate(query, response)
            redis_helpers.redis_set(
                self.redis_conn,
                query,
    ...
    def agent_redis_lookup(self, query):
        response = redis_helpers.redis_get(
            self.redis_conn, query, "redis_lookup_response", verbose=self.verbose
        )
    ...
    def agent_cog_search(self, query):
        response = redis_helpers.redis_get(
            self.redis_conn, query, "cog_search_response", verbose=self.verbose
        )

        if response is None:
            if USE_COG_VECSEARCH:
                response = "\n\n".join(
                    cog_vecsearch(query, self.cogsearch_filter_param)
                )
            else:
    ...
    def agent_cog_lookup(self, query):
        response = redis_helpers.redis_get(
            self.redis_conn, query, "cog_lookup_response", verbose=self.verbose
        )
    ...
    def agent_bing_search(self, query):
        if self.use_bing or (USE_BING == "yes"):
            response = redis_helpers.redis_get(
                self.redis_conn, query, "bing_search_response", verbose=self.verbose
            )
    ...
    def evaluate(self, query, context):
        if self.evaluate_step:
            completion_enc = openai_helpers.get_encoder(CHOSEN_COMP_MODEL)
            max_comp_model_tokens = openai_helpers.get_model_max_tokens(
                CHOSEN_COMP_MODEL
            )
            ...
            empty_prompt = len(
                completion_enc.encode(
                    utils.langchain_helpers.mod_react_prompt.mod_evaluate_instructions.format(
                        context="", question="", todays_time="", history=""
                    )
                )
            )
            allowance = (
                max_comp_model_tokens - empty_prompt - MAX_OUTPUT_TOKENS - query_len
            )
            if self.verbose:
                print(
                    "Evaluate Call Tokens:",
                    len(completion_enc.encode(context)),
                    allowance,
                )
            context = completion_enc.decode(completion_enc.encode(context)[:allowance])
            prompt = utils.langchain_helpers.mod_react_prompt.mod_evaluate_instructions.format(
                context=context,
                question=query,
                todays_time=self.get_date(""),
                history=self.history,
            )
            if self.verbose:
                print("Evaluate OAI Call")
            response = openai_helpers.contact_openai(
                prompt, CHOSEN_COMP_MODEL, MAX_OUTPUT_TOKENS, verbose=self.verbose
            )
    ...
    def qc(self, query, answer):
        prompt = utils.langchain_helpers.mod_react_prompt.mod_qc_instructions.format(
    ...
    def chichat(self, query):
        prompt = (
            utils.langchain_helpers.mod_react_prompt.mod_chit_chat_instructions.format(
                question=query
            )
        )
        if self.verbose:
    ...
    def unified_search(self, query):
        response = redis_helpers.redis_get(
            self.redis_conn, query, "response", verbose=self.verbose
        )
        ...
        if response is None:
            ...
            completion_enc = openai_helpers.get_encoder(CHOSEN_COMP_MODEL)
            response = completion_enc.decode(
                completion_enc.encode(response)[:MAX_SEARCH_TOKENS]
            )
    ...
    def specific_search(self, q, func_name):
        if func_name == "redis_search":
            return redis_search(q, self.redis_filter_param)
        if func_name == "cog_lookup":
            return cog_lookup(q, self.cogsearch_filter_param)
        if func_name == "cog_search":
            return cog_search(q, self.cogsearch_filter_param)

        if USE_BING == "yes":
            if func_name == "bing_lookup":
                return self.bing_search.run(q)
    ...
    def get_history(self, prompt_id):
        try:
            ...
            if (prompt_id is None) or (prompt_id == ""):
                ...
                rhist = redis_helpers.redis_get(
                    self.redis_conn, prompt_id, "history", verbose=self.verbose
                )
                if rhist is None:
                    hist = ""
                else:
                    ...
                    for i in range(len(new_hist)):
                        if new_hist[i] == "":
                            continue
                        if new_hist[i].startswith("System: "):
                            continue
                        if new_hist[i].startswith("AI: "):
                            continue
                        self.memory.save_context(
                            {"input": new_hist[i].replace("Human: ", "")},
                            {"output": new_hist[i + 1].replace("AI: ", "")},
                        )
                        if self.verbose:
                            print(
                                "Saving Context:",
    ...
    def generate_history_messages(self, hist):
        ...
        if self.verbose:
            print("Chat History Messages", messages)  # TODO
        return messages
    ...
    def manage_history(self, hist, sources, prompt_id):
        ...
        if self.verbose:
            print("Generated new history", hist)
        ...
        if hist_enc_len > MAX_HISTORY_TOKENS * 0.85:
            ...
            hist = openai_helpers.openai_summarize(hist, CHOSEN_COMP_MODEL).replace(
                "<|im_end|>", ""
            )
    ...
    def process_request(self, query, hist, pre_context):
        ...
        try:
            if self.agent_name == "ccr":
                ...
                response = OldSchoolSearch().search(
                    query,
                    hist,
                    pre_context,
                    filter_param=self.redis_filter_param,
                    enable_unified_search=self.enable_unified_search,
                    lc_agent=self,
                    enable_cognitive_search=self.enable_cognitive_search,
                    evaluate_step=self.evaluate_step,
                    stream=self.stream,
                    verbose=self.verbose,
                )
            else:
                response = self.zs_chain(
                    {"input": query, "history": hist, "pre_context": pre_context}
                )

        except Exception as e:
            e_str = str(e)
            return (
                "I am sorry, I am not able to find an answer to your question. Please try again with a different question.",
                [],
                [f"Technical Error: {e_str}"],
            )
    ...
    def output_partial_answer(self):
        ...
        if self.connection is not None:
            self.connection["socketio"].emit(
                "token",
                self.partial_answer.replace("\n", "<br>"),
                to=self.connection["connection_id"],
            )
        self.partial_answer = ""
        self.num_partial_answer = 0
    ...
    def get_pre_context(self, intent):
        if (intent is None) or (intent == ""):
            ...
            pre_context = redis_helpers.redis_get(
                self.redis_conn, intent, "answer", verbose=self.verbose
            )
            sources = redis_helpers.redis_get(
                self.redis_conn, intent, "sources", verbose=self.verbose
            )
    ...
    def get_intent(self, query):
        prompt = utils.langchain_helpers.mod_react_prompt.mod_extract_intent_instructions.format(
            question=query
        )
        if self.verbose:
        ...
        try:
            ...
            intent, keywords = (
                intent.group(0).replace("\n", "").replace("Intent:", "").strip(),
                keywords.group(0).replace("\n", "").replace("Keywords:", "").strip(),
            )
            intent, keywords = (
                intent.replace(",", "").strip(),
                keywords.replace(",", "").replace(".", "").strip(),
            )
    ...
    def run(self, query, redis_conn, prompt_id=None, filter_param=None):
        ...
        if self.check_adequacy:
            while tries > 0:
                ...
                if adequate == "no":
                    answer, sources, likely_sources = self.process_request(
                        query, hist, pre_context
                    )
                    tries -= 1
                else:
                    break
