...
from utils import bot_helpers, km_agents, langchain_helpers, redis_helpers
...
def on_config(agent_type):
    ...
    agent = km_agents.KMOAI_Agent(
        ...
        params_dict=global_params_dict,
        stream=True,
...
def handle_message(q):
    ...
    answer, sources, likely_sources, s_id = agents_sid[request.sid].run(
        q, redis_conn, request.sid
    )
    sources_str = ""
    send(answer)
    if len(sources) > 0:
...
def process_kmoai_request(req):
    ...
    return bot_helpers.openai_interrogate_text(
        query,
        session_id=session_id,
        filter_param=filter_param,
        agent_name=search_method,
